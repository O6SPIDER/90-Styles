...
const StoreNavbar: React.FC<StoreNavbarProps> = ({ cartCount, onCartToggle, onSearch }) => {
  ...
  return (
    <>
      ...
      <nav className="sticky top-0 left-0 w-full z-50 backdrop-blur-xl">
        <div className="flex items-center w-[92%] md:w-[85%] mx-auto mt-4 rounded-full
          ...
          <div className="hidden md:flex items-center space-x-4">
            <motion.button aria-label="Toggle theme" onClick={toggleTheme} whileTap={{ rotate: 180, scale: 0.9 }} className="p-2 rounded-full bg-gray-200 dark:bg-gray-800 hover:scale-110 transition-transform duration-300">
              {theme === "light" ? <Sun className="h-5 w-5 text-yellow-500" /> : <Moon className="h-5 w-5 text-gray-100" />}
            </motion.button>

            <motion.button onClick={onCartToggle} whileTap={{ scale: 0.9 }} className="relative p-2 rounded-full bg-gray-200 dark:bg-gray-800 hover:scale-110 transition-transform duration-300">
          ...
          <div className="flex items-center space-x-4 md:hidden">
            ...
            <button aria-label="Toggle menu" onClick={() => setIsMenuOpen(!isMenuOpen)} className="text-gray-800 dark:text-gray-200 focus:outline-none">
      ...
      {/* Mobile search */}
      <div className="md:hidden mt-20 w-[85%] mx-auto relative" ref={mobileSearchRef}>
        ...
        {isMenuOpen && (
          ...
          >
            <div className="flex flex-col items-center space-y-4">
              <button onClick={onCartToggle} className="flex items-center space-x-2 p-2 rounded-full bg-gray-200 dark:bg-gray-800 w-full justify-center hover:scale-105 transition-transform duration-300">
                <ShoppingCart className="h-5 w-5 text-gray-800 dark:text-gray-100" />
